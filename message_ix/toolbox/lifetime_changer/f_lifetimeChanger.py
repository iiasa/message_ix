# -*- coding: utf-8 -*-
"""
This function extends or shortens the lifetime of an existing technology in a MESSAGE scenario. 
The function can be used for adding or removing vintage years as well.
The method for estimation of missing values is based on linear inter/extrapolation of the two adjacent data points from available data.
A sample input data is below this code. The modified parameters are collected in a dictionary called "results" for further evaluation.
"""
import pandas as pd
import numpy as np
import ixmp as ix
...
def change_lifetime(
    ...
    '''
    sc: object
        ixmp model/scenario instance
    tec: string
        Technology name
    lifetime: integer (default: None)
        New technical lifetime. If None, the existing lifetime remains unchanged.
    year_start: integer (default: None)
        The first vintage year for the new lifetime. If None, the existing first vintage year will be used.
    year_end: integer (default: None)
        The last vintage year for the new lifetime. If None, the existing last vintage year will be used.
    ...
    par_exclude: string or a list of strings (default: None)
        Parameters to be excluded from being updated for the new lifetime (e.g., some bounds may not be needed.)
    remove_old: boolean (default: False)
        A flag for permitting the removal of old data from scenario (useful for removing some vintage years) 
    test_run: boolean (default: True)
        If true, the script is run only for test, and no changes will be commited to the MESSAGE scenario
    extrapol_neg: integer or None (default: 0.5)
        Treatment of negative values from extrapolation of two positive values: None does nothing (negative value accepted),
        and integer acts as a multiplier of the adjacent value for replacing the negative value (e.g., it can be 0, 0.5 or 1) 
    '''
# ------------------------------------------------------------------------------
    ...
    # I.2) Utility function for slicing a MultiIndex dataframe (df) and setting a
    # value to a specific level
    # idx: list of indexes, level: string, locator: list, value:
    # integer/string
    ...
    # I.3) Utility function for unifroming the "unit" of a MESSAGE parameter in different 
    # years to prevent mistakes in indexing and grouping in the main function
    def unit_uniform(df):
...
# ______________________________________________________________________________
    ...
    first_modelYear = int(sc.set('cat_year', {
...
# % III) Modification of parameters
    for node in nodes:
        ...
        # III.1) First, modifying parameter "technical_lifetime"
        ...
        if df_old.empty:
            print('There is no technical lifetime for technology "' +
                  tec + '" in node "' + node + '", no changes applied!')
            continue
        ...
        act_years = [max([x for x in vtg_years if x < lifetime + y])
                     for y in vtg_years]   # New last activity years (for new vintage year)

        df_new = df_old.copy()
        df_new = df_new.loc[df_new['year_vtg'].isin(vtg_years)]
        # For estimating required number of the loop later
        ...
        print(Fore.RESET + '> Parameter "' + parname +
            '" updated for new lifetime of "' + tec + '" in "' + node + '".')

        # Estimating how many time steps will possibly have missing data (for
        # using in the loop in Part III.3)
        n = 1 if lifetime <= lifetime_old else int(
            (lifetime - lifetime_old) / min(np.diff(vtg_years)))

# ------------------------------------------------------------------------------
        # III.2) Modification of parameters with one index related to time (e.g., "inv_cost")
        for parname in set(parlist_1d) - set(par_exclude):
            ...
            # Converting data to a pivot table for applying extrapolation
            df2 = df_old.copy().pivot_table(index=idx, columns=year_ref, values='value')

            # Preparing for backward extrapolation if more than one new vintage years
            # to be added before the existing ones
            ...
            for yr in [x for x in year_list if yr >= first_modelYear]:
                ...
                if extrapol_neg and not df2[yr].loc[(
                    ...
                    # Negative values after extrapolation are checked, if
            ...
            '" updated for new lifetime of "' + tec + '" in "' + node + '".')
# ------------------------------------------------------------------------------
        # III.3) Modification of parameters with two indexes related to time (e.g., 'input')
        for parname in set(parlist_2d) - set(par_exclude):
            ...
            if col_nontec:
                ...
                # Raising a warning if, for example, some commodities/emissions related to the same
                # technology have different vintage years (it is up to the user how to treat this).
                if not df_count.empty:
                    print(Fore.RED +
                          '>>> WARNING <<<: In parameter "' + parname +
            ...
            # lifetime extended
            # A utility for synching the index of two dataframes
                ...
                for yr in sorted(
                    ...
                    if yr < max(df_old['year_act']):
                        ...
                        if yr_act:
                            ...
                            df_yr[yr_act] = intpol(f3(df2, idx, year_ref, [year_next], yr)[yr_act],
                                                   f2(f3(df2, idx, year_ref, [year_nn], yr)[yr_act],
                                                      f3(df2, idx, year_ref, [year_next], yr)), year_next, year_nn, yr)

                        # To make sure there is no data entry in active years
                        # beyond the end year
            ...
            if yr_list:
                for yr in yr_list:
                    ...
                    # Excluding parameters with two time indexes, with no data across
                    # all active years
                    if parname not in ['relation_activity']:
                        if not df_yr.loc[pd.isna(df_yr[year_next])].empty:
                                df_yr.loc[pd.isna(df_yr[year_next]), year_next] = f3(
                                        df2, idx, year_ref, [year_next], yr).loc[:, year_next].copy()
                        if not df_yr.loc[pd.isna(df_yr[yr])].empty:
                                df_yr.loc[pd.isna(df_yr[yr]),yr] = intpol(
            ...
            for yr in vtg_years:           # The final mask to remove extra values
                df2.loc[df2.index.get_level_values(year_ref).isin(
                    [yr]), df2.columns > act_years[vtg_years.index(yr)]] = np.nan

            df_new = pd.melt(df2.reset_index(), id_vars=idx, value_vars=[x for x in df2.columns if x not in idx],
                                                var_name='year_act', value_name='value').dropna(subset=['value'])
...
# %%
# Sample input data inserted through this script

# IV.1) Loading a MESSAGE model/scenario instance


model = 'CD_Leaks_SSP2'
scen = 'baselime'
version = 10
sc = mp.Scenario(model, scen, version)

# ------------------------------------------------------------------------------
# IV.2) Defining input parameters for the main function
technology = 'coal_ppl'
lifetime = 40
# Optional entries
year_start = 1985
year_end = None
nodes = ['R14_RUS']
#par_exclude = [x for x in sc.par_list() if any(y in x for y in ['bound_','ref_'])]
par_exclude = []

# ------------------------------------------------------------------------------
# IV.3) Finding associated technologies for changing their lifetime as well
add_tecs = []
# a) Adding cooling technologies
cool_com = list(set(sc.par('input',
                           {'node_loc': nodes[0],
                            'technology': technology,
                            'level': 'cooling'})['commodity']))
if cool_com:
    cool_tec = list(set(
        sc.par('output', {'node_loc': nodes[0], 'commodity': cool_com})['technology']))
add_tecs = add_tecs + cool_tec

# b) Adding any other technology to be updated together with the specified technology
#add_tecs = add_tecs + [other_tecs_you_want]

# ------------------------------------------------------------------------------
# IV.4) Modifying the lifetime of all the technologies
for tec in [technology] + add_tecs:
    results = change_lifetime(
        sc,
        tec,
        lifetime,
        year_start,

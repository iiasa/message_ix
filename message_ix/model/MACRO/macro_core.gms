...
* ---------------------------------
* The capital constraint as in the first equation specifies the allocation of total production among current consumption :math:`\text{C}_{n, y}`, investment :math:`\text{I}_{n, y}` into building up capital stock, and 
...
* region considering the trade balance: :math:`\text{import_cost}_{r, y} - \text{export_cost}_{n, y}`. The production function in MACRO only accounts for non-energy producing parts of the 
* economy and the energy system is modeled separately in MESSAGE. In addition, MACRO does not include the trade of normal goods. Therefore, the revenues from fuel or emission certificate trade should
* be separately added to the total regional income. This is assured by including the trade balance in the :math:`\text{EC}_{n, y}`. As described in :cite:`Manne-Richels-1992`, 
...
* .. math:: \text{EC}_{n, y} = \text{COST_NODAL}_{n, y} + \text{import_cost}_{n, y} - \text{export_cost}_{n, y} \qquad \forall{n, y}

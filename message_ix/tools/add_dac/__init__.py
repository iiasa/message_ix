...
def generate_df(
    ...
    if not filepath:
        ...
        path = os.path.join(package_path, 'add_dac/DAC_all_data.yaml')          # join the current working directory with a filename
                    ...
                    if len(set_elements_dict[idx]['data']) != 0:
                        ...
                        if tech_data['model_data'].get(tec,{}).get(par,{}).get(idx,{}) != {}:
                            ...
                            if idx in ['node_loc','mode']: # to not including 'World' and 'all' mode
                                par_idx[tec][par][idx] = list(scenario.set(set_elements_dict[idx]['name']))[1:]
                            else:
                                par_idx[tec][par][idx] = list(scenario.set(set_elements_dict[idx]['name']))
                        # check and add set if not already exist
                        for e_idx in par_idx[tec][par][idx]:
                            if e_idx not in set(scenario.set(set_elements_dict[idx]['name'])):
...
def add_dac(
    ...
    node_loc = [e for e in scenario.set('node') if e not in ['World','R11_GLB']]

...
class LPdiag:
    ...
    def mps_sum(self):
        ...
        #    {"row": self.mat_row, "col": self.mat_row, "val": self.mat_val}   # this is the original line
            {"row": self.mat_row, "col": self.mat_col, "val": self.mat_val}    # this is Yoga edited line
        )
        ...
        print('[YOGA TEST 10] self.mat:\n', self.mat)
        print('[YOGA TEST 11]: writing dataframe to excel')
        row_name = pd.DataFrame(self.seq_row).transpose()[0].to_dict()
        col_name = pd.DataFrame(self.seq_col).transpose()[0].to_dict()
        matrix = (self.mat['val'].set_index(['row','col'],drop=True)
                  .unstack().fillna(0).rename(index=row_name,columns=col_name))
        #with pd.ExcelWriter('coeff_matrix.xlsx') as writer:  
        #    self.mat.to_excel(writer, sheet_name='Matrix')
        #    df_seq_row_name.to_excel(writer, sheet_name='Row_Name')
        #    df_seq_col_name.to_excel(writer, sheet_name='Col_Name')
        cmatrix = matrix.to_xarray()
        cmatrix.to_netcdf('matrix.nc')
    ...
    def add_coeff(self, words, n_line):
        ...
        print('[YOGA TEST 5]',
              '\nROW, COL, VAL: ', row_seq, col_seq, val)
    ...
    def out_loc(self, small=True, thresh=-7, max_rec=500):
        ...
        df1.reset_index()
        print('[YOGA TEST 2] df:\n', df)
        print('[YOGA TEST 3] df1:\n', df1)
        print('[YOGA TEST 4] self.mat:\n', self.mat)
        ...
        col_out.sort()
        print('[YOGA TEST 1]| col_out',col_out)
        print('[YOGA TEST 1]| col_name',self.seq_col)

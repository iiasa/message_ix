...
class LPdiag:
    ...
    def mps_sum(self):
        ...
        matrix = (matrix.set_index(['row','col'],drop=True)[['val']]
    ...
    def out_loc(self, small=True, thresh=-7, max_rec=500):
        ...
        # recreate bad coefficient dataframe for rescaler
        row_name = pd.DataFrame(self.seq_row).transpose()[0].to_dict()
        col_name = pd.DataFrame(self.seq_col).transpose()[0].to_dict()
        matrix_prob = df1.copy()
        
        matrix_prob = (matrix_prob.set_index(['row','col'],drop=True)[['val']]
                  .rename(index=row_name,level='row')
                  .rename(index=col_name,level='col'))
        
        matrix_prob_name = 'matrix_prob_lower.csv' if small else 'matrix_prob_upper.csv'
        matrix_prob.to_csv(matrix_prob_name,index=True)

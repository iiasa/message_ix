...
def check_local_model(model, notebook, shell=False):
    mp = ixmp.ixPlatform(dbtype='HSQLDB')
    if model in mp.model_scenario_list().model_name.unique():
        mp.close_db()
        return
...
@contextmanager
def read_ds(platform, name, scen):
    mp = platform
    close = mp.dbtype == 'HSQLDB'
    if close:
        mp.open_db()
    ds = mp.datastructure(name, scen)
    yield ds
    if close:
        mp.close_db()
...
@contextmanager
def scenario_ds(platform, country, name, base_scen, scen):
    mp = platform
    close = mp.dbtype == 'HSQLDB'
    if close:
        mp.open_db()
    base_ds = mp.datastructure(name, base_scen)
    ds = base_ds.clone(

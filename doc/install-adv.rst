...
A `Java Runtime Environment (JRE) <https://en.wikipedia.org/wiki/Java_(software_platform)#Java_Runtime_Environment>`_ is required to use the current default :class:`ixmp.backend.jdbc.JDBCBackend` in the :mod:`ixmp` package that handles data storage for :mod:`message_ix`.

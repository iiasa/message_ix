...
reliability_factor, reliability_bin, peak_load_factor
flexibility_factor, renewable_capacity_factor, renewable_potential
...
map_ren_grade(node,commodity,grade,year_all)$(
         SUM(level_renewable,
              renewable_potential(node,commodity,grade,level_renewable,year_all)))   = yes;

* mapping of renewable techonologies to commodities and ratings
map_ren_rating(node,inv_tec,commodity,level,rating,year_all)$(
         SUM(time,
             reliability_factor(node,inv_tec,year_all,commodity,level,time,rating))) = yes;

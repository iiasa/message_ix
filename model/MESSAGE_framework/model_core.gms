...
FIRM_CAPACITY_SHARE(node,inv_tec,commodity,level,year,rating,time)$(
    ...
    =L= reliability_bin(node,inv_tec,year,commodity,level,time,rating) * COMMODITY_USE(node,commodity,level,year)
;
...
FIRM_CAPACITY_PROVISION(node,inv_tec,year,commodity,level)$(
    ...
    SUM( (rating,time)$( map_ren_rating(node,inv_tec,commodity,level,rating,year) AND
                   reliability_factor(node,inv_tec,year,commodity,level,time,rating) ) ,
         CAP_FIRM(node,inv_tec,commodity,level,year,rating) )
...
*
*   .. math::
*      \sum_{t,y^V,m} flexibility\_factor_{n,t,y^V,y,m,c,l,t}  \cdot \\
*      \sum_{n,h}(output_{n,t,y^V,y,m,n,c,l,h,h} + input_{n,t,y^V,y,m,n,c,l,h,h}) \cdot \\
*        duration\_time\_rel_{h,h} \cdot ACT_{n,t,y^V,y,m,h} \geq 0
*
***
OPERATING_RESERVE_CONSTRAINT(node,year,commodity,level,time)..
    SUM( (tec,vintage,mode), flexibility_factor(node,tec,vintage,year,mode,commodity,level,time) *
